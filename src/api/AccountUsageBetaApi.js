...
export default class AccountUsageBetaApi {
    ...
    */
     ...
     * @param {String} opts.projectKey A project key. If specified, `environmentKey` is required and results apply to the corresponding environment in this project.
     * @param {String} opts.environmentKey An environment key. If specified, `projectKey` is required and results apply to the corresponding environment in this project.
    ...
    /**
     * Callback function to receive the result of the getExperimentationUnitsUsage operation.
     * @callback module:api/AccountUsageBetaApi~getExperimentationUnitsUsageCallback
     * @param {String} error Error message, if any.
     * @param {module:model/SeriesIntervalsRep} data The data returned by the service call.
     * @param {String} response The complete HTTP response.
     */
     ...
     * @param {String} opts.projectKey A project key. If specified, `environmentKey` is required and results apply to the corresponding environment in this project.
     * @param {String} opts.environmentKey An environment key. If specified, `projectKey` is required and results apply to the corresponding environment in this project.
     * @param {module:api/AccountUsageBetaApi~getExperimentationUnitsUsageCallback} callback The callback function, accepting three arguments: error, data, response
     * data is of type: {@link module:model/SeriesIntervalsRep}
     */
    getExperimentationUnitsUsage(opts, callback) {
      opts = opts || {};
      let postBody = null;
     ...
     * @param {Object} opts Optional parameters
     * @param {String} opts.from The data returned starts from this timestamp. Defaults to seven days ago. The timestamp is in Unix milliseconds, for example, 1656694800000.
     * @param {String} opts.to The data returned ends at this timestamp. Defaults to the current time. The timestamp is in Unix milliseconds, for example, 1657904400000.
     ...
     * @param {String} opts.groupby If specified, returns data for each distinct value of the given field. Can be specified multiple times to group data by multiple dimensions (for example, to group by both project and SDK). Valid values: project, environment, sdktype, sdk, anonymous, contextKind, sdkAppId
     * @param {String} opts.aggregationType If specified, queries for rolling 30-day, month-to-date, or daily incremental counts. Default is rolling 30-day. Valid values: rolling_30d, month_to_date, daily_incremental
     * @param {String} opts.contextKind Filters results to the specified context kinds. Can be specified multiple times, one query parameter per context kind. If not set, queries for the user context kind.
